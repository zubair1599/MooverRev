...
this.Room = (function() {
  ...
  Room.prototype.getGuid = function () {
  ...
  Room.prototype.addItem = function(item) {
    return this.setItemCount(item, this.getQuantity(item, Inventory.options) + 1);
  };
  ...
  Room.prototype.compareAddendmums = function(itemAddendum,roomItemAddendum) {

      var firstaddendums = _.map(itemAddendum, function (addendum) {
          return addendum.DisplayName;
      });

      var secondaddendums = _.map(roomItemAddendum, function (addendum) {
          return addendum.DisplayName;
      });

      if (firstaddendums.length === secondaddendums.length) {
          return _.difference(firstaddendums, secondaddendums).length === 0;
      } else {
          return false;
      }

  };
  Room.prototype.setItemCount = function(item, count, storageCount, additionalOptions, roominventoryitemid) {
    var el, sort;
    storageCount = storageCount || 0;
    el = _.find(this.Items, function(i) {
        return i.Item.ItemID === item.ItemID && (Room.prototype.compareAddendmums(additionalOptions, i.AdditionalInfo) || roominventoryitemid === i.RoomInverntoryItemID);
    });
    if (el) {
      if (el.Count > 0) {
      ...
      sort = Math.max(_.max(_.pluck(this.Items, "Sort")), -1) + 1;
        el = new Room_Item_Rel({
            ...
            RoomInverntoryItemID: this.getGuid(),
            Notes: ""
        });
      this.Items.push(el);
    }
    ...
    return this.retemplate(item,el.RoomInverntoryItemID);
  };

  

  Room.prototype.renderBodyTemplate = function (parentClass, roomInverntoryItemId) {
    var bodyContent, bodyTemplate, item;
    bodyTemplate = Inventory.templates.roomBody;
    item = $.extend({ RoomInverntoryItemID: roomInverntoryItemId }, this, {
  ...
  Room.prototype.renderUnassigned = function() {
    ...
    return rooms.map((function(_this) {
      return function(i) {
    ...
    Room.prototype.moveItem = function(toRoom, item, roominverntoryitemid) {
        var additional, count, storage;
        var additionalitem = _.filter(this.Items, function(i) {
            return i.RoomInverntoryItemID === roominverntoryitemid;
        });
        if (additionalitem.length > 0) {
            additional = additionalitem[0].AdditionalInfo;
        }
        count = this.getQuantity(item, null, roominverntoryitemid) + toRoom.getQuantity(item, additional);
        storage = this.getStorage(item) + toRoom.getStorage(item);

        this.setItemCount(item, 0, 0, null, roominverntoryitemid);
        return toRoom.setItemCount(item, count, storage, additional);
    };

  Room.prototype.retemplate = function (item, roomInverntoryItemId) {
    ...
    bodyContent = $($.parseHTML(this.renderBodyTemplate(null,roomInverntoryItemId)));
    ...
    Room.prototype.getQuantity = function(item, additionalOptions, roomInventoryItemId) {
        var resp;
        if (roomInventoryItemId) {
            resp = _.find(this.Items, function (i) {
                return i.RoomInverntoryItemID === roomInventoryItemId;
            });
        } else {
            resp = _.find(this.Items, function (i) {
                return i.Item.ItemID === item.ItemID && Room.prototype.compareAddendmums(additionalOptions, i.AdditionalInfo);
            });
        }
